...
class DeepQAgent(AgentWithConverter):
    ...
    def train(self,
        ...
        name = datetime.now().strftime("%I:%M%p, %d %B")
        wandb.init(project='L2RPN_large', name=name)
        loss_list=[]
        alive_list=[]
        reward_list=[]
        ...
        while training_step < iterations:
            ...
            #self.epsilon = self._training_param.get_next_epsilon(current_step=training_step-990000)#!!!!!!!!
            #print(training_step-999000,self.epsilon)
            ...
            # now train the model
            loss = self._train_model(training_step)
            if not loss:
            ...
            alive_list.append(alive_frame)
            reward_list.append(temp_reward)

            if training_step % 1000 == 0 or training_step == iterations - 1:
                
                wandb.log({"current_loss": np.mean(loss_list[-5:]),
                           "alive_frame": np.mean(alive_list[-5:]), 
                           "rewards": np.mean(reward_list[-5:]),
